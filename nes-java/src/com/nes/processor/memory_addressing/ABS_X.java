 ...
 * @author Dmitry
 */
public class ABS_X extends AbstractMemoryAdressing {

    private final RegExpExtractor extractor = new RegExpExtractor("\\$([0-9A-F]{1,4})\\s*,\\s*X");
    ...
    public void calculateIndex(ALU alu) {
        ...
        int x = AluUtils.unsignedByte(alu.getX());
        index =implicitOffset+x;
    }
    ...
    public String getStringDefinition(ALU alu) {
        ...
        sb.append("$").append(AluUtils.i16ToHexString(implicitOffset)).append(",X @ ");
